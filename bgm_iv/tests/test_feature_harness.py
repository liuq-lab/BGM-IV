...
the held-out criterion, the results rows, and the certify-only restore path.
No mcmc here (connectivity of the sampler is covered in test_certify).
"""
...
def test_feature_runner_restores_pixel_checkpoint_and_certify_only(fake_mnist, tmp_path):
    params = _feature_params(tmp_path)
    run_params, outputs = _run(params)
    ...
    # certify-only: restore the stage-2 checkpoint, no training
    certify = _feature_params(
        tmp_path,
        pixel_checkpoint_dir=pixel["output_dir"],
        pixel_checkpoint_timestamp=pixel["timestamp"],
        _certify_only_timestamp=stage2_timestamp,
    )
    certify_run_params, certify_outputs = _run(certify)
    assert certify_outputs["provenance"]["certify_only"] is True
    assert certify_outputs["provenance"]["checkpoint_timestamp"] == stage2_timestamp
    assert certify_outputs["provenance"]["weights"] == outputs["provenance"]["weights"]
    assert certify_outputs["final_results"]["map"] == pytest.approx(outputs["final_results"]["map"], rel=1e-3)
