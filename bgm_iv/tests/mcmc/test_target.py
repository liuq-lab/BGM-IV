...
@pytest.mark.parametrize("family", ["demand", "vector"])
...
def test_model_posterior_legality_is_fail_closed():
    ...
    with pytest.raises(ValueError, match="event-sum"):
        resolve_target(_make_model("vector", "mean"), AffinePreprocessorSpec.identity_map(785))
    with pytest.raises(ValueError, match="certifiable"):
        resolve_target(_make_model("mnist"), AffinePreprocessorSpec.identity_map(787))
...
def test_bnn_and_non_unit_powers_are_rejected():
    ...
    with pytest.raises(ValueError, match="stochastic"):
        TargetSpec(
            family="demand", blocks=(prior,), global_power=1.0,
