"""Frozen-batch vectorized HMC over the latent posterior.

One compiled graph serves every batch of one fitted target; chain and target
axes are factorized by construction; batch membership, epoch and seed are
frozen and hashed, so a segment replays bitwise on the same device.
...
class FrozenHMCError(RuntimeError):
    """Base error for a frozen-batch contract violation."""
...
class LatentPosteriorEvaluator:
    ...
    def __init__(self, resolved_target: Any):
        ...
        if str(self.spec.family) not in {"demand", "vector"}:
            raise FrozenHMCError("the batch evaluator supports the demand and vector families")
        ...
        expected = (
            ...
            else ("prior_z", "time", "vector_proxy")
        )
...
class TrajectoryPolicy:
    ...
    policy_id: str = "batch_common_stateless_uniform_support"
    ...
    def validate(self) -> "TrajectoryPolicy":
        ...
        if self.policy_id != "batch_common_stateless_uniform_support":
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "selection": "one_state_independent_L_per_batch_transition",
...
class ProductionConfig:
    ...
    def validate(self) -> "ProductionConfig":
        ...
        _finite_float("max_energy_diff", self.max_energy_diff, positive=True)
        return self
...
class ProductionSegment:
    ...
    is_accepted: np.ndarray             # [T,C,B]
    log_accept_ratio: np.ndarray        # [T,C,B]
    energy_error: np.ndarray            # [T,C,B]
    has_nonfinite: np.ndarray           # [T,C,B]
    has_extreme_energy_error: np.ndarray # [T,C,B], symmetric |delta_H| gate
    has_divergence: np.ndarray          # [T,C,B]
    has_numerical_anomaly: np.ndarray   # [T,C,B], conservative union gate
    trajectory_length: np.ndarray       # [T,C,B]
    seed: np.ndarray                    # [2]
    step_size: np.ndarray               # [C,B,1]
    state_variance: np.ndarray          # [B,D]


def _finite_per_chain_target(value: tf.Tensor) -> tf.Tensor:
    tensor = tf.convert_to_tensor(value)
    finite = tf.math.is_finite(tensor)
    rank = tensor.shape.rank
    if rank is None or rank < 2:
        raise FrozenHMCError("proposal diagnostic tensor must have rank at least 2")
    if rank == 2:
        return finite
    return tf.reduce_all(finite, axis=tuple(range(2, rank)))


def _proposal_finite(kernel_results: Any) -> tf.Tensor:
    """Per-(chain, target) flag: every proposal quantity is finite."""
    proposed = kernel_results.proposed_results
    if len(proposed.grads_target_log_prob) != 1:
        raise FrozenHMCError("runner requires a one-part tensor state")
    components = (
        tf.convert_to_tensor(kernel_results.proposed_state, tf.float32),
        tf.convert_to_tensor(proposed.target_log_prob, tf.float32),
        tf.convert_to_tensor(proposed.grads_target_log_prob[0], tf.float32),
        proposed.log_acceptance_correction,
        *tuple(proposed.initial_momentum),
        *tuple(proposed.final_momentum),
    )
    return tf.reduce_all(
        tf.stack([_finite_per_chain_target(value) for value in components]), axis=0
    )
...
class FrozenVectorizedHMC:
    ...
    def __init__(
        ...
        *,
        ...
        if not bool(getattr(evaluator, "axes_separable", False)):
            raise FrozenHMCError("target evaluator must certify target/chain separability")
        try:
        ...
        self.kernel_contract = {
            ...
            "rng_contract": "one_stateless_seed_per_frozen_batch_segment",
            ...
            "divergence": {
                "energy_error": "-log_accept_ratio",
                "nonfinite": (
                    "proposal_state/target/gradient/momentum/correction/log_ratio"
                ),
                "divergence_rule": "nonfinite_or_energy_error_gt_threshold",
                "symmetric_numerical_gate": (
                    "nonfinite_or_abs_energy_error_gt_threshold"
                ),
                "endpoint_not_full_integrator_path": True,
            },
        }
    ...
    def _build_production_graph(self):
        ...
        support = tf.constant(support_tuple, tf.int32)
        n_support = len(support_tuple)
        t_size = int(cfg.segment_size)
        threshold = tf.constant(float(cfg.max_energy_diff), tf.float32)
        ...
        def production_graph(state, step, variance, context, mask, seed):
            ...
            arrays = (
                tf.TensorArray(tf.float32, t_size),  # draws
                tf.TensorArray(tf.bool, t_size),     # accepted
                tf.TensorArray(tf.float32, t_size),  # log ratio
                tf.TensorArray(tf.float32, t_size),  # energy error
                tf.TensorArray(tf.bool, t_size),     # nonfinite
                tf.TensorArray(tf.bool, t_size),     # symmetric energy outlier
                tf.TensorArray(tf.bool, t_size),     # divergence
                tf.TensorArray(tf.bool, t_size),     # numerical anomaly union
                tf.TensorArray(tf.int32, t_size),    # trajectory length
            )

            def cond(index, *_):
                return index < t_size

            def body(index, current, *trace_arrays):
                ...
                next_u, accepted, log_ratio, proposal_finite = tf.switch_case(
                    support_index,
                    branch_fns=tuple(make_branch(kernel) for kernel in kernels),
                )
                log_finite = tf.math.is_finite(log_ratio)
                nonfinite = tf.logical_not(
                    tf.logical_and(proposal_finite, log_finite)
                )
                energy = -log_ratio
                extreme_energy = tf.abs(energy) > threshold
                # Divergence follows the Stan convention (delta_H > threshold);
                # the symmetric gate also flags enormous negative delta_H.
                divergent = tf.logical_or(nonfinite, energy > threshold)
                numerical_anomaly = tf.logical_or(nonfinite, extreme_energy)
                values = (
                    next_u * scale,
                    accepted,
                    log_ratio,
                    ...
                    divergent,
                    numerical_anomaly,
                    tf.fill([c, m], tf.gather(support, support_index)),
                )
                updated = tuple(
                    array.write(index, value)
                    for array, value in zip(trace_arrays, values)
                )
                return (index + 1, next_u, *updated)

            result = tf.while_loop(
                cond,
                body,
                loop_vars=(tf.constant(0, tf.int32), initial_u, *arrays),
                parallel_iterations=1,
            )
            return tuple(array.stack() for array in result[2:])
    ...
    def run_segment(
        ...
        *,
        ...
        arrays = {
            name: np.asarray(value.numpy()) for name, value in zip(names, outputs)
        }
        b = epoch.batch.active_size
        for name in names:
            arrays[name] = np.ascontiguousarray(arrays[name][:, :, :b])
        draws = arrays["draws"].astype(np.float32, copy=False)
        ...
        return ProductionSegment(
            ...
            draws=draws,
            is_accepted=arrays["is_accepted"].astype(bool, copy=False),
            log_accept_ratio=arrays["log_accept_ratio"].astype(
                np.float32, copy=False
            ),
            energy_error=arrays["energy_error"].astype(np.float32, copy=False),
            has_nonfinite=arrays["has_nonfinite"].astype(bool, copy=False),
            has_extreme_energy_error=arrays["has_extreme_energy_error"].astype(
                bool, copy=False
            ),
            has_divergence=arrays["has_divergence"].astype(bool, copy=False),
            has_numerical_anomaly=arrays["has_numerical_anomaly"].astype(
                bool, copy=False
            ),
            trajectory_length=arrays["trajectory_length"].astype(
                np.int32, copy=False
            ),
            seed=seed,
            step_size=step.copy(),
