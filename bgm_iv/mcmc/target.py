"""Latent inference targets of fitted BGM-IV models.
...
MCMC draw is always attributable to one exact target.
"""
...
@dataclass(frozen=True)
class TargetSpec:
    """Immutable description of one latent MCMC target.

    The identity hashes the family, every evidence block, the global power,
    decoder state, preprocessor and training provenance.
    """
    ...
    def __post_init__(self):
        if self.family not in {"demand", "vector", "mnist"}:
        ...
        if bad_power:
            raise ValueError(f"MCMC targets require unit block powers; got {bad_power}")
        if self.target_kind == "model_posterior":
            ...
            if bad_kind:
                raise ValueError(
                    f"model posterior cannot contain generalized scores: {bad_kind}"
                )
...
def resolve_target(
    ...
    *,
    ...
    """Resolve a fitted model into its immutable event-sum MCMC target.

    Parameters
    ----------
    model : BGM_IV, BGM_IV_Vector or BGM_IV_Image
    ...
    global_power : float
        Fixed to one by the production recipes.
    ...
    if str(model.params.get("covariate_block_scale", "sum")) != "sum":
        raise ValueError("MCMC inference requires event-sum covariate blocks")
        ...
        blocks.extend(
            [
                ...
                EvidenceBlockSpec(
                    ...
                    observation_support="[0,1]^784 fractional BCE score",
                ),
            ]
        )
        if int(model.extra_noise_dim) > 0:
            blocks.append(
                EvidenceBlockSpec(
                    name="noise",
                    role="observation",
                    event_size=int(model.extra_noise_dim),
                    event_reduction="sum",
                    power=1.0,
                    evidence_kind="log_likelihood",
                    observation_support=(
                        f"R^{int(model.extra_noise_dim)}, diagonal Gaussian"
                    ),
                )
            )
    spec = TargetSpec(
        family=family,
        target_kind=(
            "generalized_gibbs" if family == "mnist" else "model_posterior"
        ),
        blocks=tuple(blocks),
...
class ResolvedTarget:
    ...
    def validate_observation(self, data_v):
        ...
        if self.spec.family == "mnist":
            pixels = finite_view[:, 1:785]
            if np.any(pixels < 0.0) or np.any(pixels > 255.0):
                raise ValueError("MNIST pixel values must lie in raw scale [0,255]")
    ...
    def unpowered_event_sums(self, data_v, data_z):
        ...
        if self.spec.family == "vector":
            ...
            return result
        image_observed = data_v[:, 1:785] / 255.0
        result["pixels"] = tf.reduce_sum(
            -tf.nn.sigmoid_cross_entropy_with_logits(
                labels=image_observed,
                logits=decoded["image_logits"],
            ),
            axis=1,
        )
        if int(self.model.extra_noise_dim) > 0:
            noise_observed = data_v[:, 785:]
            result["noise"] = tf.reduce_sum(
                _gaussian_event_log_score(
                    noise_observed,
                    decoded["noise_mean"],
                    decoded["noise_var"],
...
def independent_formula_oracle(model, data_v, data_z, *, global_power=1.0):
    ...
    total = prior + time + image
    if int(model.extra_noise_dim) > 0:
        noise = tf.reduce_sum(
            gaussian_score(
                data_v[:, 785:], decoded["noise_mean"], decoded["noise_var"]
            ),
            axis=1,
        )
        total = total + noise
    return tf.cast(global_power, tf.float32) * total
