...
class CausalBGM_IV_Vector(CausalBGM_IV):
    ...
    def __init__(self, params, timestamp=None, random_seed=None):
        ...
        if int(params.get("v_dim", 0)) != 1 + self.vector_dim:
            raise ValueError("`CausalBGM_IV_Vector` requires `v_dim == 1 + vector_dim`.")
        if int(params.get("w_dim", 0)) != 1:
            raise ValueError("`CausalBGM_IV_Vector` requires `w_dim == 1`.")
