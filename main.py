...
def _load_mcmc():
    """Import the certification package on first use.
    ...
    applies the sampler's settings only once certification starts.
    """
    from bgm_iv.mcmc import certify as certify_module
    from bgm_iv.mcmc import target as target_module

    global FAMILY_RECIPES, certify_grid, AffinePreprocessorSpec, FeaturePreprocessorSpec
    global PCAFeaturePreprocessorSpec, execution_environment
    FAMILY_RECIPES = certify_module.FAMILY_RECIPES
    certify_grid = certify_module.certify_grid
    ...
    execution_environment = certify_module.execution_environment
    return certify_module
...
certify_grid = _LazyName("certify_grid")
...
def _build_arg_parser():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "run the structural evaluation / mcmc certification on it."
        ),
    )
    parser.add_argument(
        "--repeat-id",
        ...
        help=(
            ...
            "with --certify-only it is the repeat whose checkpoint is restored."
        ),
    )
    parser.add_argument(
        "--set",
...
# results.csv: one wide row per repeat — headline structural MSEs (map /
# encoder point readouts, certified mcmc integral, paired readouts on the
# certified subset), the training-side held-out criterion, and the run
# provenance that makes a row reproducible (checkpoint, ordered weight
# hashes, gamma, floors, determinism/monitor switches, device).
_FINAL_RESULT_COLUMNS = (
    ...
    "structural_mse_mcmc",
    "structural_mse_map_certified_subset",
    "structural_mse_encoder_certified_subset",
    "mcmc_num_reportable",
    "mcmc_num_batches",
    "mcmc_certified_query_fraction",
    "holdout_iv_mse_map",
...
# certified_results.csv: one row per repeat that ran mcmc — the complete
# certification summary (gate states, pooled precision, sampler evidence,
# predictive calibration on both channels, identities).
_CERTIFIED_RESULT_COLUMNS = (
    "repeat_id",
    "checkpoint_timestamp",
    "mcmc_family",
    "num_targets",
    ...
    "certified_plugin_mse",
    "certified_u_corrected_mse",
    "certified_integration_penalty",
    "certified_pooled_mcse",
    "certified_pooled_halfwidth95",
    "map_certified_subset_mse",
    "encoder_certified_subset_mse",
    "map_all_rows_mse",
    "encoder_all_rows_mse",
    "rank_rhat_max",
    "folded_rhat_max",
    "bulk_ess_min",
    "tail_ess_min",
    ...
    "divergences_total",
    "pilot_divergences",
    "unscored_plugin_mse_all_batches",
    "cov50",
    "cov80",
    "cov95",
    "width80",
...
def _build_final_results_row(params, history, final_results, provenance=None):
    """One wide row per repeat (see `_FINAL_RESULT_COLUMNS`).

    Whichever methods were evaluated fill their columns; absent methods leave
    the column empty, so map-vs-mcmc comparison at identical seeds is a
    same-row read-off.  `structural_mse_mcmc` is empty when no batch was
    REPORTABLE (an uncertified posterior never becomes a number).
    """
    ...
    certification = final_results.get("_mcmc")
    if certification is not None:
        aggregate = certification["aggregate"]
        row["structural_mse_mcmc"] = _blank(aggregate["certified_plugin_mse"])
        paired = aggregate.get("paired", {})
        row["structural_mse_map_certified_subset"] = _blank(
            paired.get("map", {}).get("certified_subset_mse")
        )
        row["structural_mse_encoder_certified_subset"] = _blank(
            paired.get("encoder", {}).get("certified_subset_mse")
        )
        row["mcmc_num_reportable"] = aggregate["num_reportable"]
        row["mcmc_num_batches"] = aggregate["num_batches"]
        row["mcmc_certified_query_fraction"] = aggregate["certified_query_fraction"]
    for key in (
        "checkpoint_timestamp",
        "checkpoint_path",
        "checkpoint_identity",
    ...
    row["certify_only"] = _blank(provenance.get("certify_only"))
    return row


def _build_certified_results_row(params, final_results, provenance=None):
    certification = final_results["_mcmc"]
    provenance = provenance or {}
    aggregate = certification["aggregate"]
    calibration = certification.get("calibration") or {}
    diagnostic = calibration.get("diagnostic_unscored") or {}
    paired = aggregate.get("paired", {})
    coverage = calibration.get("coverage_mean") or {}
    diag_coverage = diagnostic.get("coverage_mean") or {}
    row = {column: "" for column in _CERTIFIED_RESULT_COLUMNS}
    row.update(
        {
            "repeat_id": int(params.get("repeat_id", 0)),
            "checkpoint_timestamp": _blank(provenance.get("checkpoint_timestamp")),
            "mcmc_family": certification["family"],
            "num_targets": certification["grid"]["num_targets"],
            "num_queries": certification["grid"]["num_rows"],
            "num_batches": aggregate["num_batches"],
            "num_reportable": aggregate["num_reportable"],
            "state_counts": json.dumps(aggregate["state_counts"], sort_keys=True),
            "certified_query_fraction": aggregate["certified_query_fraction"],
            "certified_plugin_mse": _blank(aggregate["certified_plugin_mse"]),
            "certified_u_corrected_mse": _blank(aggregate["certified_u_corrected_mse"]),
            "certified_integration_penalty": _blank(aggregate["certified_integration_penalty"]),
            "certified_pooled_mcse": _blank(aggregate["certified_pooled_mcse"]),
            "certified_pooled_halfwidth95": _blank(aggregate["certified_pooled_halfwidth95"]),
            "map_certified_subset_mse": _blank(paired.get("map", {}).get("certified_subset_mse")),
            "encoder_certified_subset_mse": _blank(
                paired.get("encoder", {}).get("certified_subset_mse")
            ),
            "map_all_rows_mse": _blank(paired.get("map", {}).get("all_rows_mse")),
            "encoder_all_rows_mse": _blank(paired.get("encoder", {}).get("all_rows_mse")),
            "rank_rhat_max": _blank(aggregate["rank_rhat_max"]),
            "folded_rhat_max": _blank(aggregate["folded_rhat_max"]),
            "bulk_ess_min": _blank(aggregate["bulk_ess_min"]),
            "tail_ess_min": _blank(aggregate["tail_ess_min"]),
            "max_iact": _blank(aggregate["max_iact"]),
            "divergences_total": aggregate["divergences_total"],
            "pilot_divergences": aggregate.get("pilot_divergences", ""),
            "unscored_plugin_mse_all_batches": _blank(aggregate["unscored_plugin_mse_all_batches"]),
            "cov50": _blank(coverage.get("0.5")),
            "cov80": _blank(coverage.get("0.8")),
            "cov95": _blank(coverage.get("0.95")),
            "width80": _blank(calibration.get("width80_mean")),
            "wasserstein1": _blank(calibration.get("wasserstein1_mean")),
            "units_calibrated": _blank(calibration.get("units_calibrated")),
            "diag_cov50": _blank(diag_coverage.get("0.5")),
            "diag_cov80": _blank(diag_coverage.get("0.8")),
            "diag_cov95": _blank(diag_coverage.get("0.95")),
            "diag_wasserstein1": _blank(diagnostic.get("wasserstein1_mean")),
            "diag_units": _blank(diagnostic.get("units")),
            "calibration_unit": (calibration.get("config") or {}).get("scoring_unit", ""),
            "run_key": certification["pipeline"]["run_key"],
            "catalog_hash": certification["grid"]["catalog_hash"],
            "target_hash": certification["pipeline"]["target_hash"],
            "config_hash": certification["pipeline"]["config_hash"],
            "recipe_hash": certification["recipe_hash"],
            "preprocessor_identity": certification["grid"]["preprocessor_identity"],
            "device_name": _blank(provenance.get("device_name")),
            "total_seconds": certification["timings"]["total_seconds"],
        }
    )
    return row
...
def _persist_demand_design_repeat_outputs(
    ...
    if certification is not None:
        _csv_writer_append_rows(
            combo_dir / "certified_results.csv",
            _CERTIFIED_RESULT_COLUMNS,
            [_build_certified_results_row(params, final_results, provenance)],
        )
    if provenance is not None or certification is not None:
        repeat_id = int(params.get("repeat_id", 0))
...
#   mcmc     certified posterior integral (bgm_iv.mcmc.certify)
...
def _restore_demand_design_model(params, timestamp, *, train=None, manifest_extra=None):
    ...
    ``manifest_extra``; any mismatch fails instead of certifying a model that
...
def _fit_or_restore_demand_design_model(
    ...
    timestamp = params.get("_certify_only_timestamp")
    if timestamp:
        print(f"Restoring checkpoint {timestamp} (certify-only; no training) ...")
...
# --- training manifest -------------------------------------------------------
# Written next to every saved checkpoint; certify-only refuses a checkpoint
...
def _load_training_manifest(params, timestamp):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"checkpoint {path.parent} has no training manifest; it cannot be certified"
        )
...
def _verify_training_manifest(model, params, train=None, extra=None):
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"checkpoint {model.checkpoint_path} has no training manifest; it cannot be certified"
        )
    with path.open("r", encoding="utf-8") as handle:
...
def _resolve_mcmc_family(params):
    ...
    if family is None:
        raise ValueError(
            f"`{params['dataset']}` has no certifiable covariate posterior "
            "(the pixel-likelihood model is not supported by `mcmc`); use "
            "Sim_Demand_Design_Mnist_Feature_IV for MNIST."
        )
    family = str(family)
...
def _run_provenance(params, model, extra=None):
    ...
    provenance = {
        ...
        "certify_only": bool(params.get("_certify_only_timestamp")),
    }
    if extra:
...
def _certify_structural_mcmc(
    ...
    *,
    ...
    """Fail-closed certification on the full evaluation grid.

    Paired MAP / encoder predictions are computed on exactly the certified
    rows, through the same preprocessor transform the sampler sees.
    """
    grid_x_sub = np.asarray(grid_x_model, np.float32).reshape(-1, 1)
    grid_v_sub = np.asarray(grid_v_raw, np.float32)
    truth_sub = np.asarray(truth_rows, np.float64).reshape(-1)
    grid_v_sub_model = preprocessor.transform(grid_v_sub)
    paired = {}
    for method in ("map", "encoder"):
        prediction = model.predict_structural(
            grid_x_sub, grid_v_sub_model, latent_method=method
        )
        if y_stats is not None:
            prediction = _inverse_transform(prediction, y_stats)
        paired[method] = np.asarray(prediction, np.float64).reshape(-1)
    ...
    record = certify_grid(
        model,
        family=family,
        grid_x_model=grid_x_sub,
        grid_v_raw=grid_v_sub,
        preprocessor=preprocessor,
        truth_original_units=truth_sub,
    ...
    for method in methods:
        if method == "mcmc":
            if certification is None:
                raise ValueError(
                    "`mcmc` needs a certification context (dataset runner)."
                )
            record = _certify_structural_mcmc(model, **certification)
            aggregate = record["aggregate"]
            results["mcmc"] = aggregate["certified_plugin_mse"]
            results["_mcmc"] = record
            certified = aggregate["certified_plugin_mse"]
            print(
                "Structural MSE [mcmc] = "
                + ("uncertified (no REPORTABLE batch)" if certified is None else f"{certified:.6f}")
                + f"  [{aggregate['num_reportable']}/{aggregate['num_batches']} batches REPORTABLE, "
                + f"certified query fraction {aggregate['certified_query_fraction']:.3f}]"
            )
            continue
        data_y_pred = model.predict_structural(
            grid_x,
...
def _finalize_demand_design_run(
    ...
    *,
    ...
    for method in methods:
        ...
        print(f"  {method}: {'uncertified' if value is None else f'{value:.6f}'}")
    print(
...
def _run_single_demand_design_iv(params):
    ...
    if normalize_before_training:
        ...
        certification = None
        if "mcmc" in methods:
            certification = _certification_context(
                params,
...
def _run_single_demand_design_mnist_feature_iv(params):
    ...
    # ---- image encoder ---------------------------------------------------------
    # Under certify-only the stage-2 manifest names the exact stage-1
    # checkpoint; a retrained trunk would silently change the feature space.
    stage2_manifest = None
    if params.get("_certify_only_timestamp"):
        stage2_manifest = _load_training_manifest(params, params["_certify_only_timestamp"])
    trunk = None
    pixel_identity = None
    if feature_map == "egm":
        ...
        if stage2_manifest is not None:
            ...
            if pixel_timestamp and (
                ...
                raise ValueError(
                    "certify-only: the yaml pixel checkpoint differs from the stage-1 "
        ...
        if stage2_manifest is not None:
            ...
            if recorded.get("checkpoint_identity") != pixel_identity["checkpoint_identity"]:
                raise RuntimeError(
                    "certify-only: restored stage-1 weights differ from those recorded "
...
def main():
    ...
    if args.certify_only:
        if args.repeat_id is None:
            raise ValueError("--certify-only requires --repeat-id.")
        params["_certify_only_timestamp"] = str(args.certify_only)
